...
void initPlayers(int player_id[2], FILE *fps[2][2]) {
    int i, fd[4][2];
    for (i = 0; i < 4; i++) {
        pipe(fd[i]);
    }
    ...
    while (1) {
        ...
        for (round = 0; round < num_of_round; round++) {
            for (int i = 0; i < 2; i++) {
                ...
                // fprintf(stderr, "r %d d %d p %d b %d\n", round, depth, players[i], bids[i]);
            }
            
            winner = (bids[0] > bids[1] ? 0 : 1);
            if (depth == 0) { // root host
                fprintf(stderr, "round %d: player %d wins with bid %d\n", round, players[winner], bids[winner]);
                fprintf(stderr, "round %d: player %d loses with bid %d\n", round, players[~winner&1], bids[~winner&1]);
